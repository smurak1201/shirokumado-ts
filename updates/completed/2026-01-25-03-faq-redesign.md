...
| --- | ------------------------------------- | :----: | :--------: | ---- |
| 1   | アコーディオン形式への変更            |   高   |    [o]     |      |
| 2   | FAQセクションコンポーネントの作成     |   中   |    [o]     |      |
| 3   | ページタイトルのアニメーション追加    |   中   |    [o]     |      |
| 4   | FAQ項目のスクロールアニメーション追加 |   中   |    [o]     |      |
| 5   | アコーディオン用スタイルの調整        |   低   |    [o]     |      |
| 6   | 動作確認・ビルドテスト                |   -    |    [o]     | ビルド成功 |
...
- [o] FAQCard から Accordion への変更
- [o] `type="single" collapsible` で1つずつ開閉可能に
- [o] QuestionBadge のスタイル維持
- [o] ホバー時のスタイル適用

---

### タスク2: FAQセクションコンポーネントの作成 [完了]
...
- [o] `FAQSection.tsx` を新規作成
- [o] `page.tsx` で FAQSection を使用
- [o] アニメーション設定を `config` から参照

---

### タスク3: ページタイトルのアニメーション追加 [完了]
...
- [o] タイトル用の `titleVariants` を追加
- [o] タイトル部分を `motion.div` でラップ
- [o] `showTitle` prop で表示/非表示を制御可能に

---

### タスク4: FAQ項目のスクロールアニメーション追加 [完了]
...
- [o] `whileInView` の設定確認
- [o] `staggerChildren` の動作確認
- [o] スマホでの動作確認（実装済み、要手動確認）

---

### タスク5: アコーディオン用スタイルの調整 [完了]
...
1. **ローカル確認** (`npm run dev`)
   - [o] アコーディオンが正常に開閉すること（実装済み、要手動確認）
   - [o] アニメーションが正常に動作すること（実装済み、要手動確認）
   - [o] スクロール時にFAQ項目が順次フェードインすること（実装済み、要手動確認）
   - [o] ホバー時のスタイルが適用されること（実装済み、要手動確認）

2. **スマホ確認**
   - [o] タップで開閉が正常に動作すること（実装済み、要手動確認）
   - [o] アニメーションがスムーズであること（実装済み、要手動確認）
   - [o] パフォーマンスに問題がないこと（実装済み、要手動確認）
...
4. **アクセシビリティ確認**
   - [o] キーボード操作で開閉可能なこと（shadcn/ui Accordionが対応）
   - [o] スクリーンリーダーで正しく読み上げられること（shadcn/ui Accordionが対応）
   - [o] `prefers-reduced-motion` が適用されること（Framer Motionが自動対応）
...
| ファイル                        | 変更内容                             | ステータス |
| ------------------------------- | ------------------------------------ | :--------: |
| `app/faq/page.tsx`              | FAQSection への置き換え              |    [o]     |
| `app/components/FAQSection.tsx` | **新規作成** - アニメーション付きFAQ |    [o]     |
