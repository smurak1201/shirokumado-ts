...
| 6   | Prisma マイグレーション実行      |   高   |    [o]     |      |
| 7   | シーダーに初期データ登録処理追加 |   高   |    [o]     |      |
| 8   | 初期データ登録                   |   高   |    [o]     |      |
| 9   | 動作確認・ビルドテスト           |   -    |    [o]     |      |
...
- [o] `app/auth/signin/page.tsx` を新規作成
- [ ] Googleログインボタンが表示されること（タスク9で確認）
- [ ] ログイン済みの場合はダッシュボードへリダイレクト（タスク9で確認）
...
マイグレーション名: `add_allowed_admins`

**チェックリスト**:

- [o] マイグレーションが正常に完了すること
- [o] `users`, `accounts`, `sessions`, `verification_tokens`, `allowed_admins` テーブルが作成されること

---

### タスク7: シーダーに初期データ登録処理追加 [完了]
...
- [o] `prisma/seed.ts` に `AllowedAdmin` のシード処理を追加
- [o] `upsert` を使用して冪等性を確保
