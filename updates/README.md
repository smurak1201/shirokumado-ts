...
3. [ファイル命名規則](#ファイル命名規則)
4. [基本構造](#基本構造)
5. [セクション詳細](#セクション詳細)（目次、折りたたみの使い方を含む）
...
**タスク数が多い仕様書では必須。** 各セクション・タスクへのアンカーリンクを設置し、クリックでジャンプできるようにする。
...
- GitHubとVS Codeのプレビューでクリック移動可能
- アンカーは見出しの小文字化 + スペースを `-` に変換した形式
- 記号（`()`, バッククォート等）はアンカーから除外される
...
**コード例が長い場合は `<details>` タグで折りたたむ。** 問題点・修正内容・チェックリストは常に表示し、実装例のみ折りたたむことで、全体を素早くスキャンできるようにする。
...
- 変更箇所は「○行目付近」と記載
- **長いコード例は `<details>` で折りたたむ**（短い変更前/変更後の比較はそのまま表示してよい）
...
- `<summary>` と内容の間に**空行が必須**（ないとMarkdownが正しくレンダリングされない）
- `</details>` の前にも**空行が必須**
- GitHub、VS Codeプレビューで折りたたみ表示される
...
- [タスク詳細](#タスク詳細)
  - [タスク1: タスク名](#タスク1-タスク名)
  - [タスク2: 動作確認・ビルドテスト](#タスク2-動作確認ビルドテスト)
- [変更対象ファイル一覧](#変更対象ファイル一覧)
- [備考](#備考)
- [実装後の更新](#実装後の更新)
