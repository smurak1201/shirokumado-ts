...
7. **前提条件を明記** - 以下の項目を確認し、該当するものは必ず記載する。前提条件が満たされているか不明な場合は仕様書の作成を中断し、ユーザーに確認する:
   - 先行する仕様書（ファイル名で参照）
   - 必要な環境変数
   - 必要なnpmパッケージ（インストール済みか否か）
   - 必要なデータベーステーブル/マイグレーション
   - 必要なCSS変数やconfig定数
...
該当するものを記載する（なければセクションごと省略可）:

- 先行する仕様書: `updates/YYYY-MM-DD-NN-タスク名.md` が完了済みであること
- 環境変数: `SITE_URL` が設定済みであること
- npmパッケージ: `パッケージ名` がインストール済みであること
- CSS変数/定数: `--header-height` が定義済みであること
```
