...
2. **コード例は実装イメージがわかる程度に** - import文や型定義を含め、どういう実装をするかが伝わる程度に記載する（自明な部分は省略可）
3. **変更箇所を明確に** - 「変更前」「変更後」を並べ、既存コードの何行目付近かを記載。新規作成は「**新規作成**」と明記
4. **タスクの依存関係を明示** - 実行順序通りに並べ、依存がある場合は備考に記載
5. **完了条件を具体的に** - 何をもって完了とするかを明記
6. **既存コードの参照を活用** - 「`app/components/ProductGrid.tsx` を参考に実装」のように記載するとスタイルを合わせやすい
...
- 長いコード例は `<details>` で折りたたみ、短い変更前/変更後はそのまま表示
