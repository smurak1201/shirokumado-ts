...
**説明**: Next.js App Router を使用して、Server Components を優先し、パフォーマンスと SEO を最適化します。詳細については、[App Router ガイド](./guides/frontend/app-router-guide.md)を参照してください。
...
**説明**: Prisma ORM を使用してデータベースアクセスを型安全に行います。クエリの最適化と N+1 問題の回避に注意します。
...
- **N+1 問題の回避**: データが N 件ある場合、`include`を使用することで 1 回のクエリで済む（[Prisma ガイド - N+1 問題の詳細解説](./guides/backend/prisma-guide.md#n1-問題の詳細解説)）
...
- **N+1 問題の発生**: データが N 件ある場合、合計 N+1 回のクエリが実行される（[Prisma ガイド - N+1 問題の詳細解説](./guides/backend/prisma-guide.md#n1-問題の詳細解説)）
...
**詳細**: `withErrorHandling`と`safePrismaOperation`はジェネリクスを使用しています。詳細については、[TypeScript ガイド - ジェネリクス](./guides/basics/typescript-guide.md#ジェネリクス)を参照してください。
...
**詳細**: ジェネリクスの詳細な説明については、[TypeScript ガイド - ジェネリクス](./guides/basics/typescript-guide.md#ジェネリクス)を参照してください。
...
このアプリでは、商品の表示順序を一括更新する際にトランザクションを使用しています。詳細は [Prisma ガイド - $transaction](./guides/backend/prisma-guide.md#transaction) を参照してください。

### TypeScript

**説明**: TypeScript を使用して型安全性を確保し、コードの品質を向上させます。詳細な使用方法については、[TypeScript ガイド](./guides/basics/typescript-guide.md)を参照してください。
...
**説明**: 統一されたエラーハンドリングシステムを使用します。エラークラス、エラーコードの定数定義、構造化ログなどが含まれます。

**詳細**: エラーハンドリングの詳細については、[ユーティリティ関数ガイド - エラーハンドリング](./guides/backend/utilities-guide.md#エラーハンドリング-liberrorsts) を参照してください。
...
**説明**: API Routes では、統一されたレスポンス形式とエラーハンドリングを使用します。リクエストバリデーションと適切な HTTP メソッドの使用を徹底します。
...
**説明**: コードの可読性を向上させるため、適切なコメントを記述します。コードから明らかに分かる情報はコメントに含めず、複雑なアルゴリズムや特殊な実装のみにコメントを追加します。

#### 基本原則

**推奨**: コードだけで意図が明確に伝わる場合、コメントは不要です。複雑なアルゴリズムや、特定のライブラリの少し特殊な使い方など、補足が必要な場合にのみ簡潔なコメントを記述してください。

**理由**:

- **可読性**: コード自体が自己文書化され、コメントが不要な場合はコードが読みやすくなる
- **保守性**: コメントとコードの不一致を防ぎ、保守が容易になる
- **簡潔性**: 不要なコメントを削除することで、コードが簡潔になる

**避ける**: コードから明らかに分かるコメント。

```typescript
// 悪い例: コードから明らかに分かるコメント
const name = "商品名"; // 商品名を設定
setFormData({ ...formData, name }); // フォームデータを更新
```

**理由**:

- **冗長性**: コードから明らかに分かる情報をコメントで繰り返すと、コードが冗長になる
- **保守性**: コードを変更した際にコメントも更新する必要があり、保守が困難になる
- **可読性**: 不要なコメントがコードの可読性を損なう

#### 機能説明のコメント

**推奨**: コンポーネント、カスタムフック、APIエンドポイントなどには、その機能や使用方法を説明するJSDoc形式のコメントを追加してください。
...
- **ドキュメント**: コードからは明確に分からない情報（コンポーネントの目的、提供する機能、使用方法など）を簡潔に記述することで、実質的なドキュメントとして機能する
- **理解の促進**: 新しい開発者がコードを理解する際の助けになる
- **使用方法の明確化**: コンポーネントやフックの使用方法が明確になる
- **保守性**: コードの意図が明確になり、保守が容易になる

**避ける**: コードから明らかに分かる情報をコメントで繰り返すこと。

```typescript
// 悪い例: コードから明らかに分かる情報をコメントで繰り返す
/**
 * 商品名を設定する関数
 * @param name - 商品名
 */
function setName(name: string) {
  setFormData({ ...formData, name }); // フォームデータのnameフィールドを更新
}
```

**理由**:

- **冗長性**: コードから明らかに分かる情報をコメントで繰り返すと、コメントが冗長になる
- **保守性**: コードを変更した際にコメントも更新する必要があり、コメントとコードの不一致が発生しやすい
- **可読性**: 不要なコメントがコードの可読性を損なう

#### 複雑なアルゴリズムのコメント

**推奨**: 複雑なアルゴリズムや、特定のライブラリの少し特殊な使い方など、補足が必要な場合にのみ簡潔なコメントを記述してください。
...
- **理解の促進**: 複雑なアルゴリズムの意図を理解する際の助けになる
- **特殊な実装の説明**: 特定のライブラリの特殊な使い方や、実装の意図を説明できる
- **保守性**: 将来の開発者がコードを理解し、修正する際の助けになる

### コンポーネント設計

**説明**: 再利用可能で保守しやすいコンポーネントを設計します。Server Components と Client Components の使い分けについては、[Next.js App Router セクション](#nextjs-app-router)の「Server Components を優先し、直接データフェッチ」を参照してください。
...
このセクションでは、プロジェクトで使用する命名規則を説明します。一貫性のある命名規則により、コードの可読性と保守性が向上します。
...
このセクションでは、プロジェクトのファイル構造と配置ルールを説明します。一貫性のあるファイル構造により、コードの可読性と保守性が向上します。
...
**詳細**: ユーティリティ関数の詳細については、[ユーティリティ関数ガイド](./guides/backend/utilities-guide.md)を参照してください。
...
このセクションでは、プロジェクトで使用する Git ワークフローを説明します。ブランチ戦略、コミットメッセージ、プルリクエストの運用方法を定義します。
...
このセクションでは、プロジェクトで使用するコミットメッセージの形式を説明します。一貫性のあるコミットメッセージにより、プロジェクトの履歴が理解しやすくなります。
...
このセクションでは、プロジェクトで実施するコードレビューの方針を説明します。コードの品質を保つため、すべての変更に対してコードレビューを実施します。
...
このセクションでは、プロジェクトで実施するテストの方針を説明します。テストの種類、ファイルの配置、書き方を定義します。
...
このセクションでは、プロジェクトで実施するパフォーマンス最適化の方針を説明します。画像最適化、コード分割、データベースクエリの最適化などを実施します。
...
このセクションでは、プロジェクトで実施するセキュリティ対策を説明します。環境変数の管理、SQL インジェクション対策、XSS 対策などを実施します。
...
**詳細**: API Routes での入力検証の実装方法については、[API Routes セクション](#api-routes)の「リクエストバリデーション」を参照してください。
...
このセクションでは、プロジェクトのドキュメントを記述する際のルールを説明します。コードブロックの参照方法、ファイルパスの記述方法などを定義します。
...
このセクションでは、コード生成時に確認すべきチェックリストを説明します。コードの品質を保つため、すべての項目を確認してください。
...
- **[ユーティリティ関数ガイド](./guides/backend/utilities-guide.md)**: エラーハンドリング、ログ、API 型定義などの詳細
- **[React ガイド](./guides/frontend/react-guide.md)**: React の包括的なガイド
- **[JSX ガイド](./guides/basics/jsx-guide.md)**: JSX の構文とベストプラクティス
- **[TypeScript ガイド](./guides/basics/typescript-guide.md)**: TypeScript の使用方法
- **[Next.js ガイド](./guides/frontend/nextjs-guide.md)**: Next.js の使用方法
- **[App Router ガイド](./guides/frontend/app-router-guide.md)**: App Router の使用方法
