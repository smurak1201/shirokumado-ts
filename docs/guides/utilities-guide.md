...
このドキュメントは「**lib/ 内のユーティリティ関数**」を説明します。商品関連、画像圧縮、Blob Storage、エラーハンドリング、環境変数など、共通関数の使い方を理解したいときに参照してください。
...
- [`app/dashboard/homepage/components/form/ProductFormFields.tsx`](../../app/dashboard/homepage/components/form/ProductFormFields.tsx): 価格フォーマット、数値入力の検証（`ProductForm`で使用）
...
1. **ブラウザのメモリ制限**: 大きな画像ファイルを読み込む際、ブラウザのメモリ制限により読み込みが失敗する可能性があります。実際のテストでは 17MB で失敗が確認されています
2. **処理時間**: 大きなファイルは圧縮処理に時間がかかり、ユーザー体験が悪化します
3. **デバイス依存**: スマートフォンなどの低スペックデバイスでは、より小さいサイズでも失敗する可能性があります
4. **安定性**: 10MB 以下であれば、デバイスやブラウザの違いに関わらず安定して処理できます

**運用時の注意事項**:

- **必ず 10MB 以下の画像を使用してください**
- 14MB 程度でも成功する場合がありますが、デバイスやブラウザによっては失敗する可能性があるため推奨しません
- 17MB 以上の画像は確実に失敗するため、使用しないでください

#### ファイルサイズによる処理方法

- **5MB 以上**: `createImageBitmap` API を使用（メモリ効率が良い）
- **5MB 未満**: Blob URL を使用（従来の方法）

#### エラーハンドリング

10MB 以上のファイルを選択した場合、警告ダイアログが表示されます：
...
圧縮後も 4MB を超える場合は、エラーメッセージが表示され、アップロードは拒否されます。

#### 設定値

ファイルサイズの制限は [`lib/config.ts`](../../lib/config.ts) で管理されています：

```typescript
imageConfig: {
  MAX_FILE_SIZE_MB: 4,                    // 最大ファイルサイズ（MB）
  MAX_FILE_SIZE_BYTES: 4 * 1024 * 1024,   // 最大ファイルサイズ（バイト）
  COMPRESSION_TARGET_SIZE_MB: 3.5,        // 圧縮後の目標サイズ（MB）
  MAX_IMAGE_WIDTH: 1920,                  // 最大幅（ピクセル）
  MAX_IMAGE_HEIGHT: 1920,                 // 最大高さ（ピクセル）
  COMPRESSION_QUALITY: 0.85,              // 圧縮品質（0.0-1.0）
}
```
...
const databaseUrl = env.DATABASE_URL; // 型安全
const blobToken = env.BLOB_READ_WRITE_TOKEN; // 型安全
```

**注意**: このアプリでは、`DATABASE_URL` が必須です。
