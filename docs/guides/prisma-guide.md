...
このドキュメントは「**Prisma ORM の使い方**」を説明します。データベース操作、クエリ、マイグレーションなど、Prisma の詳細を理解したいときに参照してください。
...
**詳細な説明**: エラーハンドリングの詳細、エラークラスの定義、統一されたエラーハンドリングシステムについては、[ユーティリティ関数ガイド - エラーハンドリング](./utilities-guide.md#エラーハンドリング-liberrorsts) を参照してください。
