...
- [実務で役立つ Tips](#実務で役立つ-tips)
  ...
  - [.gitignore の活用](#gitignore-の活用)
    ...
    - [パターン記法一覧](#パターン記法一覧)
    - [除外の取り消し（!）](#除外の取り消し)
    - [プロジェクト別の典型例](#プロジェクト別の典型例)
    - [注意点](#注意点)
    - [.gitignore のテンプレート](#gitignore-のテンプレート)
...
コミット前に必ず差分を確認しましょう。意図しない変更の混入を防ぐことができます。具体的なコマンドは[状態確認](#状態確認)を参照してください。

**確認の流れ**:

1. `git status` で変更ファイルの一覧を確認
2. `git diff` で変更内容を確認
3. `git add` で必要なファイルだけをステージング
4. `git diff --staged` でステージング内容を最終確認
5. `git commit` でコミット
