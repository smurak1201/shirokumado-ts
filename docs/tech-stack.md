...
このドキュメントは「**どの技術を使っているか**」を説明します。技術スタックの全体像や選定理由を理解したいときに参照してください。

**関連ドキュメント**:
- [プロジェクト構造](./project-structure.md): ファイル配置のリファレンス
- [開発ガイドライン](./development-guide.md): コーディング規約
- [勉強用ガイド](./guides/learning-guide.md): 各技術の学習順序
...
- [開発ツール](#開発ツール)
  - [リンター](#リンター)
  - [ビルドツール](#ビルドツール)
- [プロジェクト構造・フロントエンド実装](#プロジェクト構造・フロントエンド実装)
- [今後の追加予定](#今後の追加予定)
- [開発コマンド](#開発コマンド)
- [参考リンク](#参考リンク)
...
- **shadcn/ui**
  ...
  shadcn/ui は、Radix UI と Tailwind CSS をベースにした、コピー&ペースト可能なコンポーネントライブラリです。コンポーネントを npm パッケージとしてインストールするのではなく、プロジェクトに直接コピーする方式を採用しています。
...
- **lucide-react** `^0.562.0`
  ...
  Lucide は、Feather Icons をベースにしたオープンソースのアイコンライブラリです。React コンポーネントとして使用でき、ツリーシェイキングに対応しています。

  **このアプリでの使われ方**: ダッシュボードや各ページのアイコン表示に使用。

- **sonner** `^2.0.7`

  sonner は、React 向けのシンプルで美しいトースト通知ライブラリです。

  **このアプリでの使われ方**: ユーザーへの操作結果通知（成功・エラー）に使用。`alert()`の代わりに`toast.success()`や`toast.error()`を使用。

- **@dnd-kit/core** `^6.3.1` / **@dnd-kit/sortable** `^10.0.0`

  dnd kit は、React 向けの軽量なドラッグ&ドロップライブラリです。

  **このアプリでの使われ方**: ダッシュボードの商品表示順序の変更（ドラッグ&ドロップで並び替え）に使用。

- **sharp** `^0.34.5`

  sharp は、Node.js 向けの高速な画像処理ライブラリです。

  **このアプリでの使われ方**: サーバーサイドでの画像処理（リサイズ、フォーマット変換等）に使用。

- **heic2any** `^0.0.4`

  HEIC（iPhone のデフォルト画像形式）を他の形式に変換するライブラリです。

  **このアプリでの使われ方**: クライアントサイドの画像圧縮処理で、HEIC 画像を WebP/JPEG に変換。
...
- **Tailwind CSS** `^4`
  ...
  **v4 の主な特徴**:
  ...
  - **CSS-first configuration**: `@theme` ディレクティブで CSS ファイル内でテーマをカスタマイズ（`tailwind.config.js` は不要）
  - **高速なビルド**: 新しいエンジンにより、フルビルドは最大5倍、インクリメンタルビルドは100倍以上高速化
  - **レスポンシブデザイン**: プレフィックス（`sm:`, `md:`, `lg:`など）で簡単にレスポンシブ対応
  - **CSS レイヤー**: カスケードレイヤーを使用し、詳細度の問題を回避
  - **一貫性**: デザイントークン（色、スペーシング、タイポグラフィ）を統一

  **このアプリでの使われ方**:

  - `@import "tailwindcss"` でシンプルにセットアップ（v4 の推奨形式）
  - `@theme inline` ブロックで shadcn/ui のテーマ変数を定義
...
- **next-auth (Auth.js v5)** `^5.0.0-beta.30`

  Auth.js（旧 NextAuth.js）は、Next.js 向けの認証ライブラリです。OAuth プロバイダーとの統合、セッション管理、データベースアダプターを提供します。

  **主な特徴**:

  - **OAuth 認証**: Google 等の OAuth プロバイダーとの簡単な統合
  - **データベースセッション**: Prisma アダプターによるセッション管理
  - **Edge 対応**: Edge Runtime での動作をサポート
  - **型安全性**: TypeScript との統合が優れている

  **このアプリでの使われ方**:

  - Google OAuth 認証によるログイン
  - 許可リスト方式によるアクセス制御
  - データベースセッション（7日間有効）
  - Proxy + Layout によるルートガード

  **詳細な使用方法については、[認証システムドキュメント](./authentication.md) と [Auth.js ガイド](./guides/backend/authjs-guide.md) を参照してください。**

- **@auth/prisma-adapter** `^2.11.1`

  Auth.js と Prisma を統合するアダプターです。セッション、ユーザー、アカウント情報を PostgreSQL に保存します。
...
- `@types/node` `^24.10.4`: Node.js の型定義
