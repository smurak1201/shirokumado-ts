...
| 種別 | quantity（個数/件数） | amount（金額） |
| ---- | -------------------- | -------------- |
| Z004 | 販売個数 | 小計（個数 x 単価） |
| Z005 | 販売個数 | 部門の売上合計 |
| Z009 | 客数（取引件数） | 時間帯の売上合計 |

Z004の例: クッキー缶2026 個数9, 金額34,020（= 単価3,780 x 9）

### 売上分析で使用する指標

| 指標 | データソース | 算出方法 |
| ---- | ------------ | -------- |
| 総売上 | Z005（部門別） | amountの合計 |
| 客数 | Z009（時間帯別） | quantityの合計 |
| 商品別売上 | Z004（商品別） | 商品ごとのamount |
| 部門別売上 | Z005（部門別） | 部門ごとのamount |
| 時間帯別売上 | Z009（時間帯別） | 時間帯ごとのamount |
| 客単価 | Z005 + Z009 | 総売上 / 客数 |

### Z001の扱い

Z001（売上明細）はSDカードに存在するため取り込みは行うが、売上分析には使用しない。
Z001は28行固定の集計サマリー（総売、純売、現金在高、税金、取引中止など）であり、
上記の指標はすべてZ004/Z005/Z009で代替できる。
オーナーからの要望があれば、Z001の特定行（例: 取引中止件数）を個別に参照する形で対応する。

### 注意事項

- Z009は存在しない日がある（全日にはない）。客数が取得できない日は0件として扱う
- Z004も存在しない日がある。商品別分析はデータがある日のみ対象
- Z005は全日にデータがあり、売上の基本ソースとして最も安定している
