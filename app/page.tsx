 ...
 *
 * 理由:
 * 1. 公開状態の自動判定: `calculatePublishedStatus()` が現在時刻を使用して公開日・終了日に基づいて
 *    公開状態を自動判定するため、常に最新の時刻で判定する必要がある
 * 2. 商品情報の頻繁な更新: 商品情報は頻繁に更新される可能性があり、常に最新のデータを表示する必要がある
 * 3. データベースから最新のデータを取得: データベースから常に最新のデータを取得する必要がある
 *
 * 静的生成（SSG）や ISR を使用すると、ビルド時や再生成時の時刻で公開状態が固定されてしまい、
 * 公開日・終了日に基づく自動判定が正しく動作しません。
 */
