 ...
 *
 * ハードコードされた値やマジックナンバーを集約し、変更や環境ごとの設定を容易にします。
 * 設定値を変更する場合は、このファイルを編集するだけで全体に反映されます。
 *
 * 主な設定カテゴリ:
 * - imageConfig: 画像アップロードと圧縮の設定
 * - blobConfig: Vercel Blob Storage の設定
 * - apiConfig: API エンドポイントのキャッシュ設定
 * - displayConfig: UI 表示の設定
 * - dndConfig: ドラッグ&ドロップの設定
 * - animationConfig: アニメーションの設定
 *
 * ベストプラクティス:
 * - マジックナンバーを避け、意味のある定数名を使用
 * - 単位（MB、秒など）を定数名に含めて明確化
 * - 関連する設定をグループ化して可読性を向上
 *
 * 注意点:
 * - `as const` を使用して、TypeScript で型を厳密に推論
 * - 環境変数ではなく定数として管理（ビルド時に確定する値）
 * - Vercel の制限（関数ペイロードサイズなど）を考慮した値を設定
 */
export const config = {
  /**
   * 画像アップロードと圧縮に関する設定
...
// - TypeScript が厳密な型推論を行い、誤った値の割り当てを防ぐ
// - 設定値が実行時に変更されないことを保証
