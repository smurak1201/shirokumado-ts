...
- **Next.js** `16.1.1`
  ...
  **このアプリでの使われ方**:
  ...
  - App Router を使用して、`app/page.tsx`（ホームページ）と `app/faq/page.tsx`（FAQ ページ）を実装
  - Server Components を活用し、データベースから直接データを取得してレンダリング（`app/page.tsx`）
  - API Routes（`app/api/`）で商品管理の CRUD 操作を実装
  - Next.js Image コンポーネントで画像の自動最適化と WebP 変換を実現
  - 動的ルーティングと静的生成を適切に使い分け、パフォーマンスを最適化
...
- **React** `19.2.3`
  ...
  **このアプリでの使われ方**:
  ...
  - カスタムフック（`useModal`, `useProductModal`）で状態管理ロジックを分離
...
- **Prisma** `^7.2.0`
  ...
  **このアプリでの使われ方**:
  ...
  - `prisma/schema.prisma` でデータベーススキーマを定義（商品、カテゴリー、画像など）
  - Prisma Client を `lib/prisma.ts` でシングルトンインスタンスとして管理
  - Server Components や API Routes から Prisma Client を使用してデータを取得・更新
  - マイグレーション機能でスキーマ変更を管理し、開発と本番環境で一貫性を保持
  - 型安全なクエリにより、実行時エラーを事前に防止
