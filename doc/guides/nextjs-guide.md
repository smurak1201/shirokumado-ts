...
- [フォント最適化](#フォント最適化)
  ...
  - [このアプリでの使用例](#このアプリでの使用例-1)
- [メタデータと SEO](#メタデータと-seo)
  - [メタデータの設定](#メタデータの設定)
  - [このアプリでの使用例](#このアプリでの使用例-2)
- [ビルドとデプロイ](#ビルドとデプロイ)
  - [ビルドコマンド](#ビルドコマンド)
  - [デプロイ](#デプロイ)
...
- **サーバーサイドレンダリング（SSR）**: サーバー側で HTML を生成し、初期表示を高速化
- **静的サイト生成（SSG）**: ビルド時に HTML を生成し、パフォーマンスを最適化
- **インクリメンタル静的再生成（ISR）**: 静的ページを段階的に再生成
- **App Router**: ファイルベースのルーティングシステム（Next.js 13 以降）
- **API Routes**: サーバーレス関数として API エンドポイントを実装
- **画像最適化**: 自動的な画像最適化と WebP/AVIF 変換
- **コード分割**: ページごとに自動的にコードを分割し、バンドルサイズを削減
- **フォント最適化**: Google Fonts などのフォントを最適化して読み込み
...
**説明**: `fill` プロパティを使用して、親要素のサイズに合わせて画像を表示します。`priority` により、優先読み込みが行われます。
...
**説明**: モーダル内の画像は `priority` により優先読み込みされます。
...
**説明**: フォント変数を `body` 要素に適用し、`antialiased` クラスでアンチエイリアスを有効化しています。
