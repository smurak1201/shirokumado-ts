...
- [セッション管理](#セッション管理)
  - [JWT セッション](#jwt-セッション)
    - [JWT（JSON Web Token）とは](#jwtjson-web-token-とは)
    - [Auth.js での JWT セッション設定](#authjs-での-jwt-セッション設定)
...
JWT は、当事者間で情報を安全に送信するためのコンパクトで自己完結型の方法を定義するオープンスタンダード（RFC 7519）です。

**JWT の構造**:

JWT は 3 つの部分で構成され、ドット（`.`）で区切られています。
...
└────────┘                               └────────────┘                        └────────────┘
     ...
     │                                          │ 4. JWT を生成（署名付き）              │
     │                                          │                                     │
     │ 5. JWT を Cookie にセットして返却          │                                     │
     │◄─────────────────────────────────────────│                                     │
     │                                          │                                     │
     │ 6. 以降のリクエスト（JWT を Cookie で送信）  │                                     │
     │─────────────────────────────────────────►│                                     │
     │                                          │                                     │
     │                                          │ 7. JWT の署名を検証                   │
     │                                          │    （データベースアクセス不要！）        │
...
- JWT は**暗号化**されていないため、ペイロードの内容は誰でも読める（Base64 デコードするだけ）
- そのため、パスワードなどの機密情報は JWT に含めてはいけない
- 署名の検証には `AUTH_SECRET` が必要なので、秘密鍵を安全に管理することが重要

#### Auth.js での JWT セッション設定
...
| ストレージ | クライアント（Cookie） | サーバー（データベース） |
| セキュリティ要件 | 標準的な用途向け | 高セキュリティ要件向け |
...
- **一般的な Web アプリケーション**: ブログ、ポートフォリオ、情報サイトなど
- **読み取り中心のアプリ**: ダッシュボードの閲覧、レポート表示など
- **高トラフィックなサービス**: データベース負荷を軽減したい場合
- **マイクロサービス構成**: 複数のサービス間で認証情報を共有したい場合
- **サーバーレス環境**: Vercel、AWS Lambda など、コールドスタートを最小化したい場合
...
- **即座のセッション無効化が必要な場合**
  - パスワード変更時に全デバイスからログアウトさせたい
  - 不正アクセス検知時に即座にセッションを無効化したい
  - ユーザーのアカウント停止を即時反映させたい

- **セッション情報を頻繁に更新する場合**
  - ユーザーのロールや権限がリアルタイムで変わる
  - サブスクリプション状態を即時反映させたい

- **高度なセッション管理が必要な場合**
  - 同時ログイン数を制限したい
  - アクティブなセッション一覧を表示したい
  - 特定のデバイスのセッションだけを無効化したい

```typescript
// ❌ このような要件には JWT は不向き
// 例: パスワード変更時に全セッションを無効化
async function changePassword(userId: string, newPassword: string) {
  await prisma.user.update({
    where: { id: userId },
    data: { hashedPassword: await hash(newPassword) },
  });

  // JWT の場合、既存のトークンは有効期限まで有効なまま
  // → セキュリティリスク！
}
```
...
- **EC サイト / 決済機能があるサービス**: セキュリティ要件が高い
- **管理者向けシステム**: 権限変更の即時反映が必要
- **金融・医療系アプリ**: コンプライアンス要件でセッション管理が必須
- **マルチデバイス対応**: ユーザーが複数デバイスのセッションを管理したい
- **Stripe などの決済連携**: ユーザー状態の即時反映が重要
...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  session: {
    strategy: "database",
    maxAge: 24 * 60 * 60, // 24時間（セキュリティ重視で短めに）
  },
});
```

#### データベースセッションを避けるべきケース（NG）

```
❌ データベースセッションを使うべきでない場面
```
...
- **サーバーレス環境でのコールドスタート問題**
  - DB 接続のオーバーヘッドが大きい
  - Vercel の Edge Runtime では使用できない
...
白熊堂プロジェクトでは、**将来的に Stripe を使った EC サイトの構築を検討している**ため、**データベースセッション**を推奨します。

**理由**:

1. **決済セキュリティ**: 不正アクセス検知時に即座にセッションを無効化できる
2. **サブスクリプション管理**: 支払い状態の変更をセッションに即時反映できる
3. **ユーザー体験**: 複数デバイスでのセッション管理が可能
4. **コンプライアンス**: EC サイトに求められるセキュリティ要件を満たせる
